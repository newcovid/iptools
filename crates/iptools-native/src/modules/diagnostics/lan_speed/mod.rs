...
use super::{FocusArea, config_field_item};
...
use proto::{
    DEFAULT_PORT, Direction, Flow, LanEvent, Proto, TestSpec, TestSummary, avg_bytes_per_sec,
    ...
    loop {
        tokio::select! {
            _ = cancellation.cancelled() => {
                *abort.lock().map_err(|error| error.to_string())? = true;
