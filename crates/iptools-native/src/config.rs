...
impl Config {
    ...
    pub fn load(path: Option<&str>) -> Self {
        ...
        if Path::new(&path).exists()
            ...
            && let Ok(mut cfg) = serde_json::from_str::<Config>(&content)
        {
            cfg.path = path;
    ...
    pub fn save(&self) {
        if let Ok(content) = serde_json::to_string_pretty(self)
            && let Err(error) = write_atomic(&self.path, content.as_bytes())
        {
            tracing::warn!(path = %self.path.display(), %error, "failed to persist configuration");
        }
    }
}
